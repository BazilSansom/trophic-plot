...
if ~isfield(opts,'ShowRowLabels');   opts.ShowRowLabels = false; end

% ---- export defaults ----
if ~isfield(opts,'Export');         opts.Export = false; end
if ~isfield(opts,'ExportBase');     opts.ExportBase = 'fig_food_web'; end
...
% (1) TFL weighted
axs(1) = nexttile(tl,1);
tflPlot(w_weighted, 'plotopts', struct('ShowLabels', opts.ShowLabels,'EdgeWidthMode','fixed','EdgeWidthScale','linear','ArrowSize',opts.ArrowSize));
setPanelTitle(axs(1), "Trophic Flow Layout (TFL)",S);
fixTiledAxesCommon(axs(1));
makeAxesDataSquare(axs(1), opts.PadFrac);

% (2) Layered (drawn using plotTFL for aesthetic consistency)
axs(2) = nexttile(tl,2);
...
axs(3) = nexttile(tl,3);
tflPlot(w_weighted,...
         'plotopts', struct('ShowLabels', opts.ShowLabels,'ArrowSize',opts.ArrowSize),...
         'layoutopts', struct('HeightMode','snapped','MinSepFrac',0.05));
setPanelTitle(axs(3), "Trophic Flow Layout (TFL) Snapped",S);
